            ...
            for (let i = 0; i < ticketCount; i++) {
                const { error } = await supabase
                    ...
                    .insert([{
                        ...
                        is_networking_enabled: isNetworkingEnabled
                    }]);
                        ...
                        >
                            <GlassTile className="p-10" interactive={false}>
                                ...
                                <div className="space-y-10">
                                    ...
                                    {/* Networking Opt-in */}
                                    {event.is_networking_enabled && (
                                        <div className="p-6 rounded-[2rem] bg-blue-500/5 border border-blue-500/10 space-y-4">
                                            <div className="flex items-center justify-between">
                                                <div className="space-y-1">
                                                    <div className="flex items-center gap-2">
                                                        <Users className="h-4 w-4 text-blue-500" />
                                                        <span className="text-sm font-black tracking-tight">Attendee Networking</span>
                                                    </div>
                                                    <p className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest leading-tight max-w-[200px]">
                                                        Connect with others and join the live event chat.
                                                    </p>
                                                </div>
                                                <button
                                                    onClick={() => setIsNetworkingEnabled(!isNetworkingEnabled)}
                                                    className={cn(
                                                        "w-12 h-6 rounded-full transition-colors relative",
                                                        isNetworkingEnabled ? "bg-blue-500" : "bg-neutral-200 dark:bg-white/10"
                                                    )}
                                                >
                                                    <div className={cn(
                                                        "absolute top-1 w-4 h-4 rounded-full bg-white transition-all",
                                                        isNetworkingEnabled ? "left-7" : "left-1"
                                                    )} />
                                                </button>
                                            </div>
                                        </div>
